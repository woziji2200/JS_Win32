...
#define ThisComponent Win32_Text
#define ThisType Type_Text
...
static void win32_window_finalizer(JSRuntime *rt, JSValue val) {
    ThisComponent *ww = JS_GetOpaque(val, win32_window_class_id);
    js_free_rt(rt, ww);
}
...
static JSValue win32_window_get_width(JSContext *ctx, JSValueConst this_val,
                                      int magic) {
    ThisComponent *ww = JS_GetOpaque2(ctx, this_val, win32_window_class_id);
    if (!ww) return JS_EXCEPTION;
    if (magic == 0) {
        RECT rect;
...
static int window_init(JSContext *ctx, JSModuleDef *m) {
    ...
    JSValue ctor;
    ctor = JS_NewCFunction2(ctx, win32_window_ctor, "Win32_Text", 4,
                            JS_CFUNC_constructor, 0);
    JS_SetConstructor(ctx, ctor, proto);
    JS_SetClassProto(ctx, win32_window_class_id, proto);
    JS_SetModuleExport(ctx, m, "Win32_Text", ctor);
    return 0;
}
...
JSModuleDef *js_init_module(JSContext *ctx, const char *module_name) {
    ...
    if (!m) return NULL;
    JS_AddModuleExport(ctx, m, "Win32_Text");
